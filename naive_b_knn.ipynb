...
# Read the CSV files
df = pd.read_csv(r"C:\Users\KASHISH RANA\Downloads\titanic\train.csv")
df_test = pd.read_csv(r"C:\Users\KASHISH RANA\Downloads\titanic\test.csv")
...
for row in test.itertuples(index=False):
    ...
    if total_yes > total_no:
        list1 = [[ans, 1]]
        output_dataframe = output_dataframe._append(pd.DataFrame(list1, columns=['Actual', 'Predicted']), ignore_index=True)
    else:
        list0 = [[ans, 0]]
        output_dataframe = output_dataframe._append(pd.DataFrame(list0, columns=['Actual', 'Predicted']), ignore_index=True)
